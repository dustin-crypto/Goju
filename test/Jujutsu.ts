...
import { Jujutsu } from "../typechain-types/contracts/Jujutsu";
import { Jujutsu__factory } from "../typechain-types/factories/contracts/Jujutsu__factory";
...
describe("Dapp", function() {
  ...
  async function deployDappFixture() {
    ...
    const BTC_ORACLE = "0xb3DF0a9582361db08EC100bd5d8CB70fa8579f4B";
    const ETH_ORACLE = "0x6850A6e773b9a625C6810E34070491d0FF97E065";
    const CRO_ORACLE = "0x4636AC8216805Fe96dE9E7aFc62dA99096a930F6";
    const DAI_ORACLE = "0xB3b3EcC7FB61C642Ea64D0AD5c61A5EAc4822717";
    const TONIC_ORACLE = "0x14f753940720C1Fa4247Cd464C7EA28c806d123F";
    const USDT_ORACLE = "0xde3fE5B7932F3Fb225d91f119735fECE12426c43";

    // const oracleAddresses: string[] = [
    //   "0xb3DF0a9582361db08EC100bd5d8CB70fa8579f4B", // BTC
    //   "0x6850A6e773b9a625C6810E34070491d0FF97E065", // ETH
    //   "0x4636AC8216805Fe96dE9E7aFc62dA99096a930F6", // CRO
    //   "0xB3b3EcC7FB61C642Ea64D0AD5c61A5EAc4822717", // DAI
    //   "0x14f753940720C1Fa4247Cd464C7EA28c806d123F", // TONIC
    //   "0xde3fE5B7932F3Fb225d91f119735fECE12426c43", // USDT
    // ];
    ...
    // deploy dapp
    const Dapp: Jujutsu__factory = await ethers.getContractFactory("Jujutsu");
    const dapp: Jujutsu = await Dapp.deploy(goju.address, VVSRouterAddress, TectonicProxyAddress);
    await dapp.deployed();
    ...
    // register token price oracle
    tx = await dapp.registerNativeCurrencyOracle(CRO_ORACLE);
    await tx.wait();
    tx = await dapp.registerOracle(WBTC, BTC_ORACLE);
    await tx.wait();
    tx = await dapp.registerOracle(WETH, ETH_ORACLE);
    await tx.wait()
    tx = await dapp.registerOracle(USDT, USDT_ORACLE);
    await tx.wait()
    tx = await dapp.registerOracle(TONIC, TONIC_ORACLE);
    await tx.wait()
    tx = await dapp.registerOracle(DAI, DAI_ORACLE);
    await tx.wait()

    const tokens = { WBTC, TUSD, USDC, WETH, DAI, USDT, TONIC };
    const tTokens = { tWBTC, tTUSD, tUSDC, tWETH, tDAI, tUSDT, tTONIC, tCRO };
    // await prepareUserWithToken(user1, USDC, "5000");
    return { dapp, goju, tokensAddr, tTokensAddr, owner, user1, tokens, tTokens, VVSRouterAddress, TectonicProxyAddress };
  }

  async function prepareUserWithToken(user: any, token: string, amount: string) {
    // const [user] = await ethers.getSigners();
    // const USDC = "0xc21223249CA28397B4B6541dfFaEcC539BfF0c59";

    const wcro: Contract = new ethers.Contract("0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23", wcroABI, user);
    const vvsRouter: Contract = new ethers.Contract("0x145863Eb42Cf62847A6Ca784e6416C1682b1b2Ae", vvsRouterABI, user);

    let tx;
    // swap cro to wcro
    // let tx = await wcro.deposit({ value: utils.parseEther("5000") });
    // await tx.wait();
    // tx = await wcro.approve(vvsRouter.address, ethers.constants.MaxUint256);
    // await tx.wait();
    // expect(await wcro.balanceOf(user.address)).to.equal(utils.parseEther("5000"));

    // swap
    const blockNumber = await ethers.provider.getBlockNumber();
    const block = await ethers.provider.getBlock(blockNumber);
    tx = await vvsRouter.swapExactETHForTokens(0, [wcro.address, token], user.address, block.timestamp + 1000000, { value: utils.parseEther(amount) });
    await tx.wait();

    // get USDC
    const tokenContract: Contract = new ethers.Contract(token, erc20ABI, user);
    const usdcBalance = await tokenContract.balanceOf(user.address);
    console.log(usdcBalance);
    expect(usdcBalance.gt(BIG_ZERO)).to.be.true;
  }
  ...
  describe("Deployment", () => {
    ...
    //   const { dapp, tokensAddr, tTokensAddr } = await loadFixture(deployDappFixture);

    //   for (let i = 0; i < tokensAddr.length; i++) {
    //     expect(await dapp.marketsMapping(tokensAddr[i])).to.equal(tTokensAddr[i]);
    //   }
    // });
    ...
    it("Should get CRO price feed", async () => {
      const { dapp } = await loadFixture(deployDappFixture);

      const croPrice = await dapp.getNativeCurrencyPriceFeed();
      expect(croPrice.gt(BIG_ZERO)).to.be.true;
    });

    it("Should get token price feed", async () => {
      const { dapp, tokens } = await loadFixture(deployDappFixture);

      // stable
      const tusdPrice = await dapp.getTokenPriceFeed(tokens.TUSD);
      const usdcPrice = await dapp.getTokenPriceFeed(tokens.USDC);

      // tokens
      const daiPrice = await dapp.getTokenPriceFeed(tokens.DAI);
      const tonicPrice = await dapp.getTokenPriceFeed(tokens.TONIC);
      const wethPrice = await dapp.getTokenPriceFeed(tokens.WETH);
      const wbtcPrice = await dapp.getTokenPriceFeed(tokens.WBTC);

      console.log({ daiPrice });
      console.log({ tonicPrice });
      console.log({ wethPrice });
      console.log({ wbtcPrice });
      expect(daiPrice.gt(BIG_ZERO)).to.be.true;
      expect(tonicPrice.gt(BIG_ZERO)).to.be.true;
      expect(wethPrice.gt(BIG_ZERO)).to.be.true;
      expect(wbtcPrice.gt(BIG_ZERO)).to.be.true;
      expect(tusdPrice.eq(BIG_10POW18)).to.be.true;
      expect(usdcPrice.eq(BIG_10POW18)).to.be.true;
    });
  });
  ...
  describe("Transactions", () => {
    ...
    async function setupUserBalanceFixture() {
      ...
      const wcro: Contract = new ethers.Contract("0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23", wcroABI, user1);
      const vvsRouter: Contract = new ethers.Contract(VVSRouterAddress, vvsRouterABI, user1);

      // Swap get USDC
      const blockNumber = await ethers.provider.getBlockNumber();
      const block = await ethers.provider.getBlock(blockNumber);
      let tx = await vvsRouter.connect(user1).swapExactETHForTokens(0, [wcro.address, tokens.USDC], user1.address, block.timestamp + 1000000, { value: utils.parseEther("4000") });
      await tx.wait();

      // approve dapp to use USDC collateral
      const USDCContract: Contract = new ethers.Contract(tokens.USDC, erc20ABI, user1);
      tx = await USDCContract.connect(user1).approve(dapp.address, ethers.constants.MaxUint256);
      await tx.wait();
    ...
    it("Should user open short and get short token", async () => {
      const { dapp, goju, user1, tokens, tTokens, USDCContract } = await loadFixture(setupUserBalanceFixture);
      ...
      // // get approval for dapp to use collateral
      // // const USDCContract: Contract = new ethers.Contract(tokens.USDC, erc20ABI, user1);
      // tx = await USDCContract.connect(user1).approve(dapp.address, ethers.constants.MaxUint256);
      // await tx.wait();
      console.log('----------- before open short')
      console.log('Monitor: user1 USDC balance', await USDCContract.balanceOf(user1.address));
      console.log('-----------')

      // call openShort on dapp
      let tx = await dapp.connect(user1).openShort(tokens.WETH, utils.parseEther("0.01"), tokens.USDC);
      await tx.wait();
      ...
      const WETHContract: Contract = new ethers.Contract(tokens.WETH, erc20ABI, user1);
      ...
      const tUSDCContract: Contract = new ethers.Contract(tTokens.tUSDC, tErc20ABI, user1);
      ...
      console.log('goju balance for user1', await goju.balanceOf(user1.address));
      ...
      tx = await goju.connect(user1).approve(dapp.address, ethers.constants.MaxUint256);
      await tx.wait();

      console.log('user USDC balance before redeem', await USDCContract.balanceOf(user1.address));
      ...
      const wethInDapp1 = await WETHContract.balanceOf(dapp.address);
      console.log('WETH in dapp', wethInDapp1);
      const tTokenInDapp1 = await tUSDCContract.balanceOf(dapp.address);
      const usdcInDapp1 = await USDCContract.balanceOf(dapp.address);
      console.log('tUSDC in dapp', tTokenInDapp1);
      console.log('USDC in dapp', usdcInDapp1);
